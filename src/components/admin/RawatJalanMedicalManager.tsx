...
import {
  createRawatJalanMedical,
  listRawatJalanMedicals,
  setRawatJalanMedicalActive,
} from '../../services/budget.services';
import type { RawatJalanMedical } from '../../types/types';
import { Alert } from '../ui/Alert';
import { Button } from '../ui/Button';

type RawatJalanMedicalManagerProps = {
  /** Tighter layout when embedded inside a modal (narrow width) */
  variant?: 'default' | 'modal';
};

export function RawatJalanMedicalManager({ variant = 'default' }: RawatJalanMedicalManagerProps) {
  const PAGE_SIZE = 6;
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [items, setItems] = useState<RawatJalanMedical[]>([]);
  ...
  async function load() {
    ...
    setError(null);
    try {
      const res = await listRawatJalanMedicals(!showInactive);
      if (!res.success) {
      ...
      const message = e instanceof Error ? e.message : 'Gagal memuat medical Rawat Jalan';
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  async function onAdd() {
    ...
    if (!trimmed) {
      setError('Nama medical wajib diisi');
      return;
    }
    setLoading(true);
    setError(null);
    try {
      const res = await createRawatJalanMedical({ name: trimmed });
      if (!res.success) {
        setError(res.message);
        return;
      }
      ...
      const message = e instanceof Error ? e.message : 'Gagal menambah medical';
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  async function toggleActive(id: string, next: boolean) {
    ...
    setError(null);
    try {
      const res = await setRawatJalanMedicalActive(id, { isActive: next });
      if (!res.success) {
        setError(res.message);
        return;
      }
      ...
      const message = e instanceof Error ? e.message : 'Gagal mengubah status medical';
      setError(message);
    } finally {
      setLoading(false);
    }
    ...
    <div className={isModal ? 'flex flex-col gap-3' : 'flex flex-col gap-4'}>
      ...
      <div className="flex flex-col gap-1">
        <div className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
          Rawat Jalan Medical Options
        </div>
        <div className="text-sm text-zinc-600 dark:text-zinc-400">
          Ini dipakai untuk dropdown saat admin input spend <span className="font-semibold">Rawat Jalan</span>.
        </div>
      </div>

      <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_auto] sm:items-end">
        <div className="flex min-w-0 flex-col gap-1">
          <label className="text-sm font-medium text-zinc-800 dark:text-zinc-200">Tambah Medical</label>
          <input
            value={name}
      ...
      <div className="min-w-0 overflow-hidden rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
        {/* Full-page only: table uses viewport breakpoints; modal always uses cards (viewport lg/md would not match modal width). */}
        {!isModal ? (
        <div className="hidden md:block">
          ...
          <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {pagedItems.length === 0 && !loading ? (
              <div className="p-6 text-sm text-zinc-600 dark:text-zinc-400">Belum ada medical.</div>
            ) : null}
        ...
        <div className={isModal ? 'block' : 'md:hidden'}>
          <div className="flex flex-col gap-2 p-3">
            {pagedItems.length === 0 && !loading ? (
              <div className="p-2 text-sm text-zinc-600 dark:text-zinc-400">Belum ada medical.</div>
            ) : null}
